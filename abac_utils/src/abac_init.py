...
def init(force):
    ...
    # create the include/exclude path scope file
    create_file(
        CONFIG_ROOT + CONFIG_PATH_SCOPE_FILE,
        force,
        "ABAC path scope file already exists",
        {"include": [SHARED_DIR], "exclude": []},
    )
    ...
    # check if the default include directory is created and leave a message to inform the admin
    if not Path(SHARED_DIR).is_dir():
        print(f"The default ABAC include path - {SHARED_DIR} is NOT created")
    else:
        print(f"The default ABAC include path - {SHARED_DIR} is created")
